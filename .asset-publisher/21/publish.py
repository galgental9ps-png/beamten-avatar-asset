...
SET = ROOT / 'BeamtenMonitorAvatar/Assets 21'
PARTS = ROOT / '.asset-publisher/21'
FILES = {
...
def materialize():
    ...
    phrase_path.write_text(
        '{"theme":"Picknicktag","line":"Ich gehe zurück zum Picknick – der Feierabendkorb ist bereits ordnungsgemäß gepackt."}',
        encoding='utf-8',
    )
...
def validate():
    ...
    assert phrase['theme'] == 'Picknicktag' and isinstance(phrase['line'], str) and len(phrase['line']) <= 180
    verified.append(str(phrase_path.relative_to(ROOT)))
    return verified


def publish():
    assert not SET.exists(), 'Assets 21 already exists before materialization'
    materialize()
    files = validate()
    run('git', 'config', 'user.name', 'github-actions[bot]')
    run('git', 'config', 'user.email', '41898282+github-actions[bot]@users.noreply.github.com')
    run('git', 'add', '--', *files)
    run('git', 'commit', '-m', 'Assets 21: Picknicktag Asset-Set')
    run('git', 'push', 'origin', 'HEAD:main')
    run('git', 'fetch', 'origin', 'main')
    run('git', 'reset', '--hard', 'origin/main')
    verified = validate()
    main_sha = subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip()
    print(json.dumps({'verified': verified, 'mainSha': main_sha}))
